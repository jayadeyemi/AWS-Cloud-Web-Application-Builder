...
phase1() {
    ...
    # Wait for the all subnets to be available
    if [[ $status -eq 0 ]]; then
    ...
    if [[ $status -eq 0 ]]; then
        execute_command "DB_ROUTE_TABLE=\$(aws ec2 create-route-table --vpc-id "$MAIN_VPC_ID" --query 'RouteTable.RouteTableId' --output text)"
        status=$?
    fi

    # Name the database route table
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 create-tags --resources "$DB_ROUTE_TABLE" --tags Key=Name,Value="$DB_ROUTE_TABLE_NAME""
        status=$?
    fi

    # Associate main route table with public subnet 1
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 associate-route-table --route-table-id "$PUB_ROUTE_TABLE_ID" --subnet-id "$PUB_SUBNET1""
        status=$?
    fi

    # Associate main route table with public subnet 2
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 associate-route-table --route-table-id "$PUB_ROUTE_TABLE_ID" --subnet-id "$PUB_SUBNET2"" 
        status=$?
    fi

    # Associate private route table with private subnet 1
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 associate-route-table --route-table-id "$PRIV_ROUTE_TABLE_ID" --subnet-id "$PRIV_SUBNET1""
        status=$?
    fi

    # Associate private route table with private subnet 2
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 associate-route-table --route-table-id "$PRIV_ROUTE_TABLE_ID" --subnet-id "$PRIV_SUBNET2""
        status=$?
    fi

    # Associate database route table with database subnet 1
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 associate-route-table --route-table-id "$DB_ROUTE_TABLE" --subnet-id "$DB_SUBNET1""
    fi

    # Associate database route table with database subnet 2
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 associate-route-table --route-table-id "$DB_ROUTE_TABLE" --subnet-id "$DB_SUBNET2""
    fi
    ...
    if [[ $status -eq 0 ]]; then
        execute_command "VCP_DETAILS=\$(aws ec2 accept-vpc-peering-connection --vpc-peering-connection-id "$PEERING_CONNECTION_ID")"
        status=$?
    fi
    ...
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 create-route --route-table-id "$PUB_ROUTE_TABLE_ID" --destination-cidr-block "$DEFAULT_VPC_CIDR" --vpc-peering-connection-id "$PEERING_CONNECTION_ID""
        status=$?
    fi

    # Create a route in the Default VPC route table to the Lab VPC
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 create-route --route-table-id "$DEFAULT_ROUTE_TABLE_ID" --destination-cidr-block "$VPC_CIDR" --vpc-peering-connection-id "$PEERING_CONNECTION_ID""
        status=$?
    fi
    
    # Create a route in the main route table to the Internet Gateway
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 create-route --route-table-id "$PUB_ROUTE_TABLE_ID" --destination-cidr-block "$INTERNET_CIDR" --gateway-id "$IGW_ID""
        status=$?
    fi
    ...
    if [[ $status -eq 0 ]]; then
        execute_command "aws ec2 create-route --route-table-id "$PRIV_ROUTE_TABLE_ID" --destination-cidr-block "$INTERNET_CIDR" --nat-gateway-id "$NAT_GW_ID""
        status=$?
    fi
