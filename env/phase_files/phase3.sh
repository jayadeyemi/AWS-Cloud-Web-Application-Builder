...
######################################
    ...
    if [[ $status -eq 0 ]]; then
        execute_command "LB_SG_INTERNET_CIDR_ACCESS=$(aws ec2 authorize-security-group-ingress --group-id "$LB_SG" --protocol tcp --port 80 --cidr "$INTERNET_CIDR" --query 'Reservations[0].Instances[0].PrivateIpAddress' --output text)"
        status=$?
    fi

    if [[ $status -eq 0 ]]; then
        execute_command "LB_SG_EC2_V1_SG_ACCESS=$(aws ec2 authorize-security-group-ingress --group-id ="$LB_SG" --protocol tcp --port 80 --source-group "$EC2_V1_SG_NAME" --query 'Reservations[0].Instances[0].PrivateIpAddress' --output text)"
        status=$?
    fi
    ...
    if [[ $status -eq 0 ]]; then
        execute_command "ASG_GROUP=\$(aws autoscaling create-auto-scaling-group --auto-scaling-group-name "$EC2_ASG_NAME" --launch-template "$LAUNCH_TEMPLATE_ID" --min-size 2 --max-size 6 --desired-capacity 2 --target-group-arns "$TG_ARN" --vpc-zone-identifier "$PRIV_SUBNET1,$PRIV_SUBNET2" --load-balancer-names "$LB_NAME" --target-tracking-configuration file://config.json)"
        status=$?
    fi
